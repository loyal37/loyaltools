...
class PanelBasicInformation(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.label(text="LoyalTools v1.4.10", icon="INFO")
        if ssmt4_available:
            ...
            layout.label(text="未检测到SSMT4，运行于独立模式", icon='OUTLINER_OB_LIGHT')
        layout.label(text=TR.translate("当前游戏预设: ") + GlobalConfig.logic_name)
        layout.label(text=TR.translate("当前工作空间: ") + (current_workspace_name or "未设置"))
        ...
        standalone_row = workspace_box.row(align=True)
        standalone_row.label(text="独立模式:")
        standalone_row.prop(global_properties, "standalone_game_preset", text="")
        standalone_row.prop(global_properties, "force_standalone_preset", text="强制", toggle=True)
        if global_properties.force_standalone_preset:
            # 强制开启时，当前生效的游戏预设来自LoyalTools独立预设而非SSMT4配置
            workspace_box.label(text="预设来源: LoyalTools独立模式", icon='INFO')
        ...
        if not ssmt4_available and global_properties.workspace_source_mode != "CUSTOM":
            # 独立模式下SYNC/SPECIFIC无法从SSMT4解析工作空间，
            # 引导用户填写自定义工作空间目录(此时会自动作为工作空间来源生效)
            workspace_box.label(text="独立模式下建议切换为\"使用自定义目录\"", icon='INFO')
            workspace_box.prop(global_properties, "custom_workspace_folder_path", text="自定义目录")

        if not current_workspace_name:
            workspace_box.label(text="未解析到工作空间，请在上方指定自定义工作空间目录", icon='ERROR')
