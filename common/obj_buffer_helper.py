...
class ObjBufferHelper:
            ...
            if d3d11_element_name.startswith("COLOR"):
                ...
                if d3d11_element_name not in color_coll:
                    if hasattr(obj.data, 'color_attributes'):
                        # LoyalTools: SNORM 颜色格式数据范围为 [-1,1]，BYTE_COLOR 会被钳位到 [0,1]，
                        # 所以补全时与导入保持一致使用 FLOAT_COLOR，其它格式保持 BYTE_COLOR 不变。
                        color_attr_type = 'FLOAT_COLOR' if 'SNORM' in d3d11_element.Format else 'BYTE_COLOR'
    ...
    def _parse_color(mesh, mesh_loops_length, d3d11_element_name, d3d11_element):
        ...
        if color_data is not None:
            ...
            # TODO 添加八面体压缩法线到R32_UINT的代码
                ...
                # LoyalTools: SNORM 颜色 (如终末地 R8G8B8A8_SNORM 的 COLOR0/1/2)，
                # 导入时以 FLOAT_COLOR 存储的 [-1,1] 浮点值，这里用和 NORMAL/TANGENT 相同的
                # SNORM 转换器编码回 int8 (转换器内部已钳位到 [-1,1])。
                result = FormatUtils.convert_4x_float32_to_r8g8b8a8_snorm(result)
    ...
    def convert_to_element_vertex_ndarray(
        ...
        for d3d11_element_name in d3d11_game_type.OrderedFullElementList:
            ...
            if data is None:
                if d3d11_element_name.startswith("UNKNOWN"):
                    # LoyalTools: UNKNOWN 占位元素 (EFMI 提取时填充的布局空隙) 在 Blender 中没有数据来源，
                    # element_vertex_ndarray 已经全零初始化，跳过即保持对应字节为全零。
                    print("UNKNOWN 占位元素 [" + d3d11_element_name + "] 导出时以全零字节填充")
                    continue
    ...
    def average_normal_color(obj,indexed_vertices,d3d11GameType:D3D11GameType,dtype):
        ...
        # 归一化到[0,1]，然后映射到颜色值
        # LoyalTools: COLOR 字段为 int8 时说明是 SNORM 格式 (如终末地 R8G8B8A8_SNORM)，
        # 需保持 [-1,1] 并量化到 [-127,127]，而不是归一化到 [0,255]。
        color_is_snorm = vb['COLOR'].dtype == numpy.int8
        if color_is_snorm:
            normalized_normals = numpy.round(numpy.clip(average_normals, -1.0, 1.0) * 127).astype(numpy.int8)
        else:
        ...
        # 将新的颜色列表转换为NumPy数组
        new_color_array = numpy.array(new_color, dtype=numpy.int8 if color_is_snorm else numpy.uint8)
